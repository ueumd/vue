...
var Observer = function Observer (value) {
  ...
  if (Array.isArray(value)) {
    // 判断有没有原型 __proto__ 。__proto__是非标准属性，在IE6-10没有改属性
    if (hasProto) {
      // 用经过增强的数组原型方法 覆盖默认的原型方法, 实现数组响应式目的
...
function defineReactive$$1 (
  ...
  Object.defineProperty(obj, key, {
    ...
    get: function reactiveGetter () {
      ...
      if (Dep.target) {
        // target存在 做依赖收集 将dep添加watcher中，也将wathcer添加到dep中
        dep.depend();
 ...
 *
 * 比如： Vue.set(arr, arr[1], 'hello vue')
 */
 ...
 *
 * 处理数组 选项 为对象的情况 (arr[1].name)， 进行依赖收集
 * arr: [1, {name: 'vue'}]
 */
 ...
 *
 * 校验组件的名称
 * 开发环境会先校验开发者传入的components选项中的组件名称是否符合要求。
 * 校验的规则在validateComponentName函数中
 */
 ...
 *组件的命名需要满足两个if中判断的条件：
 ...
 2. 不能是内置的标签名，跳到isBuiltInTag函数，发现目前内置的标签名是slot和component。
 * @param name
 */
